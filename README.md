...
Na tela de configuração de bloco, um acordion **Nunca praticados** (fechado por padrão — dá para desligar em Ajustes → Geração) lista tópicos do edital sem nenhuma questão respondida, do maior peso para o menor (`lacunasDoEdital` em [`src/lib/topicos.ts`](src/lib/topicos.ts)) — mas só dentro de matéria que você já pratica e domina no geral (pelo menos uma resposta e acerto acima de 50%). Matéria nunca aberta, ou ainda fraca no geral, não entra: ali o ponto cego é a matéria inteira, não um tópico específico, e a sugestão certa seria "estude a matéria", não "responda esta questão". Um toque aplica matéria e tópico na configuração.
